...
rirls.spls.stab <- function(X, Y, lambda.ridge.range, lambda.l1.range, ncomp=1, 
     ...
     grid.resampling <- as.matrix( Reduce("rbind", mclapply(1:nresamp, function(id.samp) {
          ...
          grid_out <- sapply(lambda.l1.range, function(lambda) {
               ...
               cv_out <- rirls.spls.tune(X=Xtrain, Y=Ytrain, lambda.ridge.range=lambda.ridge.range, 
               ...
               fit_out <- rirls.spls(Xtrain=Xtrain, Ytrain=Ytrain, lambda.ridge=cv_out$lambda.ridge.opt, 
                                     lambda.l1=lambda, 
